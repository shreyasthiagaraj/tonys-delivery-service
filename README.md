...
- **Audio** — a WebAudio step-sequencer plays a tarantella-flavored loop (tempo rises with level), and the scooter engine is fully synthesized: detuned oscillators + noise grit + a putt-putt tremolo whose pitch, brightness and volume track your speed and jump on boost. Key one-shot effects (breadstick pickup, lane whoosh, boost surge, crash, pothole bump, delivery jingle) are ElevenLabs-generated sound effects embedded as base64 mp3, decoded once into AudioBuffers — with the procedural blips kept as fallbacks. Still one self-contained file.
